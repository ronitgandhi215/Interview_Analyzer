...
with st.sidebar:
    ...
    st.write(
        "1. Choose a question.\n"
        "2. Type your answer or switch to voice input.\n"
        "3. Click Analyze answer.\n"
        "4. Save your best answers to track progress."
    )
    st.markdown("---")
    ...
    st.write("• Clear feedback and analysis on demand")
    st.write("• More actionable feedback for every answer")
...
QUESTION_CATEGORIES = {
    ...
    "Behavioral": [
        ...
        "Describe an instance when you had to learn something quickly.",
    ],
    "Career": [
...
def _history_download_data() -> tuple[bytes, str, str]:
    df = _history_dataframe()
    csv = df.to_csv(index=False).encode("utf-8")
    json_text = df.to_json(orient="records", indent=2)
    return csv, json_text, "history_export"
...
with col_q:
    ...
    if selected_q == "Custom question...":
        ...
        display_q = custom_q.strip() or "Custom question"
        if not custom_q.strip():
            st.caption("Type your own question above before saving or analyzing.")
    else:
        display_q = selected_q

    st.markdown(f'<div class="question-display">{clean(display_q)}</div>', unsafe_allow_html=True)
...
if analyze_btn:
    ...
    if not final_text:
        ...
        if final_score>=75:
            grade,gbg,gcol = "Excellent","#EEF3EC","#3A7D44"
        elif final_score>=55:
            grade,gbg,gcol = "Good","#F5F0E8","#8B6914"
        elif final_score>=35:
            grade,gbg,gcol = "Fair","#FDF4EE","#B05D2A"
        else:
            grade,gbg,gcol = "Needs Work","#FCEEED","#A83232"
        ...
        history_entry = {
            ...
            "sentiment_score": sent_r["score"],
            "confidence_score": conf_r["score"],
            "communication_score": comm_r["score"],
            "final_score": final_score,
            "ml_prediction": ml_result["predicted_class"] if ml_result else "N/A",
        }
        # History saving feature removed per user request.
...
if st.session_state.history:
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div class="section-heading">Answer history</div>', unsafe_allow_html=True)
    history_df = _history_dataframe()
    if not history_df.empty:
        summary_df = history_df[["timestamp", "category", "question", "final_score", "ml_prediction", "word_count"]]
        st.dataframe(summary_df.sort_values(by="timestamp", ascending=False), hide_index=True)
        csv_data = history_df.to_csv(index=False).encode("utf-8")
        json_data = history_df.to_json(orient="records", indent=2)
        c1, c2, c3 = st.columns(3)
        with c1:
            st.download_button("Export history as CSV", csv_data, "interview_history.csv", "text/csv", key="download_csv")
        with c2:
            st.download_button("Export history as JSON", json_data, "interview_history.json", "application/json", key="download_json")
        with c3:
            if st.button("Clear history", key="clear_history"):
                st.session_state.history = []
                st.success("History cleared.")

        with st.expander("View saved answers"):
            for entry in reversed(st.session_state.history):
                st.markdown(f"**{entry['timestamp']} — {entry['question']}**")
                st.markdown(f"*Score: {entry['final_score']} — ML: {entry['ml_prediction']}*")
